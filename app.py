...
def main():
    ...
    st.title('Dashboard de Visitas - Cubo Itaú')
    df = carregar_dados()
    if df is None:
    ...
    with col3:
        st.markdown(f'<div class="modern-card"><div class="card-label">Anfitriões NÃO Notificados</div><div class="big-number">{anfitrioes_nao_notificados(df)}</div></div>', unsafe_allow_html=True)
    with col4:
    ...
    with col2:
        ...
        if not tabela_frequentes.empty:
            ...
            st.write('Nenhum visitante frequente encontrado para o período selecionado.')
...
def gerar_pptx(df, df_filtro):
    ...
    for i, (label, value) in enumerate([
        ...
        ("Anfitriões NÃO Notificados", anfitrioes_nao_notificados(df)),
